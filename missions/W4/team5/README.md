...
W4M2 과제의 질문은 "이 택시 데이터가 사람이 아니라 자율주행차 데이터라면 어떤 Data Product를 만들 수 있을까"였다.

이를 현실 문제로 좁히면 로보택시가 실제로 운행되고 있다고 가정했을 때 그 운영을 뒷받침하려면 어떤 데이터가 필요한가라는 문제가 된다. 이 분석은 NYC FHVHV trip 데이터를 로보택시 수요 데이터로 놓고 이 질문에 답한다.
...
일반 택시는 운전자가 필요할 때 직접 충전하지만, 자율주행차 플릿은 운영사가 모든 차량의 충전과 배차를 함께 관리해야 한다. 충전소 위치는 차량의 이동 경로와 운영 효율에 직접 영향을 미치는 핵심 운영 요소다. Stanford 연구팀이 충전소 입지와 배차를 함께 최적화하는 모델을 연구하고([arXiv:2107.00165](https://arxiv.org/abs/2107.00165)) CPUC 규제 데이터 기준 Waymo의 공차운행 비율이 2025년 중반 이후로도 43~45% 수준에 머무르는 것도([Findings, 2026](https://findingspress.org/article/161870-millions-of-trips-waymo-empty-miles-california-s-first-thousand-days-of-commercial-robotaxi-service)) 충전소 위치가 그만큼 운영에 중요하기 때문이다. 충전 인프라가 부족한 zone에 차량을 배치하면 빈 차 상태로 다른 zone까지 이동해야 해 운영 비용이 늘고 서비스 효율도 떨어진다.

### ③ 타겟은 Terawatt Infrastructure로 설정했다
...
이 중 두 번째 유형인 **Terawatt Infrastructure**를 타겟 고객으로 설정했다. Terawatt는 최근 Waymo 공급용 depot 확장을 위해 3억 달러 규모의 투자를 유치했다([Bloomberg](https://www.bloomberg.com/news/articles/2026-06-24/waymo-charging-partner-terawatt-raises-up-to-300-million-in-debt)). Uber 역시 휴스턴에 4MW·충전기 40기 규모의 로보택시 전용 depot을 구축하고 있어([TechCrunch](https://techcrunch.com/2026/06/17/uber-will-bring-its-premium-robotaxi-service-to-houston-in-2027/)) 충전 인프라 시장 자체가 빠르게 성장하고 있다.

**"Terawatt Infrastructure는 어느 지역에 먼저 충전 인프라를 구축해야 하는가?"**
...
- **포트당 필요 부지면적**: 324 sqft (충전 스톨 162 sqft × 동선 여유 배수 2) / (1 sqft ≒ 0.03 평)
