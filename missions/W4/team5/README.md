...
"서비스를 자율주행으로 운영한다면 충전 인프라를 어디에 얼마나 갖춰야 하는가"를 분석했다.
...
일반 택시는 운전자가 필요할 때 직접 충전하지만 자율주행차 플릿은 운영사가 모든 차량의 충전 시점을 관리해야 한다. Stanford 연구팀은 충전소 입지와 배차를 함께 최적화하는 모델을 제안했고([arXiv:2107.00165](https://arxiv.org/abs/2107.00165)) CPUC 규제 데이터를 분석한 결과 Waymo의 공차운행(deadheading) 비율은 2025년 중반 이후로도 43~45% 수준을 유지하고 있다([Findings, 2026](https://findingspress.org/article/161870-millions-of-trips-waymo-empty-miles-california-s-first-thousand-days-of-commercial-robotaxi-service)). 충전 인프라가 부족한 zone에 차량을 배치하면 빈 차 상태로 다른 zone까지 이동해야 해 운영 비용이 늘고 서비스 효율도 떨어진다.
...
*왼쪽: 6개월 합계 기준 시간대별 전체 수요 — 출퇴근보다 저녁 17\~19시에 피크가 몰린다. 오른쪽: 수요 상위 20개 zone × 시간대 평균 승차 건수 히트맵 — LaGuardia·JFK 공항이 하루 종일 압도적으로 진하다.
...
대신 `charging_model_2_ranked_by_ROI.csv`의 **ROI 상위 5개 zone**을 보면 흥미로운 패턴이 나온다 — 5곳 모두 **Queens** 소재다. 토지가가 Manhattan의 5분의 1 수준이라 같은 수요에도 투자비가 훨씬 적게 든다.
